                       ...
                       bool ensure_king_is_safe)
{
    ...
    if ((a_ascii <= x1 - 1 && x1 - 1 <= a_ascii + 7) &&
        chess[char(x1 - 1)][y1].has_figure() &&
        chess[char(x1 - 1)][y1].get_figure().is_pawn() &&
        chess[char(x1)][y1].get_figure().is_black() !=
            chess[char(x1 - 1)][y1].get_figure().is_black() &&
        chess[char(x1 - 1)][y1].get_figure().double_step0() &&
        (x2 == x1 - 1) && (y2 == y1 + decider))
    {
        returning_value = 2;
    }
    ...
    {
        if (x1 == x2 &&
            ((y2 - y1 == 1 * decider) || (y2 - y1 == 2 * decider)) &&
            !c2.has_figure())
        {
            if (y2 - y1 == 2 * decider)
                          ...
                          Chessboard& chess)  // moves on the board
{
    ...
    if (position.y ==
        int(4.5 +
            3.5 * decider))  // If pawn has reached the end of the board
    ...
    for (int i = 1; i <= (first_step ? 2 : 1); i++)
    {
        if (correct_step(chess[x][y], chess[x][y + i * decider], chess))
        {
            ...
            Circle* tempc =
                new Circle{chess[x][y + i * decider].center(), c_size / 4};
    ...
    for (int i = -1; i <= 1; i += 2)
    {
        ...
        {
            int a = correct_step(chess[x][y],
                                 chess[char(int(x) + i)][y + decider * 1],
                                 chess);
            if (a)
            {
                first_step = first_step_reserved;  // Костыль
                ...
                if (a == 1)
                    tempf = new Frame{
                        chess[char(int(x) + i)][y + decider].center(),
                        chess};
                if (a == 2)
                    tempf = new Frame{
                        chess[char(int(x) - 1)][y + decider].center(),
                        chess};
                if (a == 3)
                    tempf = new Frame{
                        chess[char(int(x) + 1)][y + decider].center(),
...
int Rook::correct_step(Cell& c1, Cell& c2, Chessboard& chess,
                       bool ensure_king_is_safe)
{
    int x1 = int(c1.location().x);
    int y1 = c1.location().y;
                                     ...
                                     VisualSteps*& steps_representation)
{
    ...
    for (int d = -1; d <= 1; d += 2)
    {
        ...
        {
            if (correct_step(chess[position.x][position.y],
                             chess[char(i)][position.y], chess))
            {
                if (chess[char(i)][position.y].has_figure())
                {
                    Frame* tempf = new Frame{
                        chess[char(i)][position.y].center(), chess};
                    steps_representation->possible_takes.push_back(tempf);
                    int sz = steps_representation->possible_takes.size();
                    chess.attach(
                ...
                else
                {
                    Circle* tempc = new Circle{
                        chess[char(i)][position.y].center(), c_size / 4};
                    tempc->set_color(chess_yellow);
                    tempc->set_fill_color(chess_yellow);
                    steps_representation->possible_steps.push_back(tempc);
                    int sz = steps_representation->possible_steps.size();
                                   ...
                                   VisualSteps*& steps_representation)
{
    ...
    for (int d = -1; d <= 1; d += 2)
    {
        ...
        {
            if (correct_step(chess[position.x][position.y],
                             chess[position.x][i], chess))
            {
                if (chess[position.x][i].has_figure())
                {
                    Frame* tempf =
                        new Frame{chess[position.x][i].center(), chess};
                    steps_representation->possible_takes.push_back(tempf);
                    int sz = steps_representation->possible_takes.size();
                    chess.attach(
                        steps_representation->possible_takes[sz - 1]);
                ...
                {
                    Circle* tempc = new Circle{
                        chess[position.x][i].center(), c_size / 4};
                    tempc->set_color(chess_yellow);
                    tempc->set_fill_color(chess_yellow);
                    steps_representation->possible_steps.push_back(tempc);
                    int sz = steps_representation->possible_steps.size();
                         ...
                         bool ensure_king_is_safe)
{
    ...
    if (!(std::abs(x2 - x1) == 1 || std::abs(x2 - x1) == 2) ||
        !(std::abs(y2 - y1) == 1 || std::abs(y2 - y1) == 2))
        return false;
    ...
    for (const Coordinate& pos : potential_steps)
    {
        if (!chess.out_of_range(pos))
        {
            if (correct_step(chess[char(x)][y], chess[pos.x][pos.y], chess))
            {
                ...
                {
                    Frame* tempf =
                        new Frame{chess[pos.x][pos.y].center(), chess};
                    steps_representation->possible_takes.push_back(tempf);
                    int sz = steps_representation->possible_takes.size();
                    chess.attach(
                        steps_representation->possible_takes[sz - 1]);
                ...
                {
                    Circle* tempc = new Circle{chess[pos.x][pos.y].center(),
                                               c_size / 4};
                    tempc->set_color(chess_yellow);
                    tempc->set_fill_color(chess_yellow);
                    steps_representation->possible_steps.push_back(tempc);
                         ...
                         bool ensure_king_is_safe)
{
    ...
    int d1,
        d2;  // d1, d2 - decider1, decider2, created to reduce copy + paste
    ...
    while (!chess.out_of_range(Coordinate{char(x), y}) && x != x2 &&
           y != y2)
    {
                        ...
                        bool ensure_king_is_safe)
{
    ...
    {
        int d1, d2;  // d1, d2 - decider1, decider2, created to reduce copy
                     // + paste
                                       ...
                                       Chessboard& chess)
{
    ...
    for (int i = 0; i < chess.N; i++)
        ...
        {
            if (correct_step(chess[position.x][position.y],
                             chess[i + a_ascii][j], chess))
            {
                if (chess[i + a_ascii][j].has_figure())
                {
                    Frame* tempf =
                        new Frame{chess[i + a_ascii][j].center(), chess};
                    steps_representation->possible_takes.push_back(tempf);
                    int sz = steps_representation->possible_takes.size();
                    chess.attach(
                        steps_representation->possible_takes[sz - 1]);
                ...
                {
                    Circle* tempc = new Circle{
                        chess[i + a_ascii][j].center(), c_size / 4};
                    tempc->set_color(chess_yellow);
                    tempc->set_fill_color(chess_yellow);
                    steps_representation->possible_steps.push_back(tempc);
                    int sz = steps_representation->possible_steps.size();
...
{
    Pawn* temp_pawn = new Pawn{chess, (is_white()) ? Figure::Type::white
                                                   : Figure::Type::black};
...
{
    King* temp_king = new King{chess, (is_white()) ? Figure::Type::white
                                                   : Figure::Type::black};
...
{
    Bishop* temp_bishop = new Bishop{
        chess, (is_white()) ? Figure::Type::white : Figure::Type::black};
...
{
    Rook* temp_rook = new Rook{chess, (is_white()) ? Figure::Type::white
                                                   : Figure::Type::black};
